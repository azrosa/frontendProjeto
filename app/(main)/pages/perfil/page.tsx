...
const Perfil = () => {
            ...
            perfilService.inserir(perfil)
                .then((response) => {
                    ...
                    toast.current?.show({
                        severity: 'error',
                        summary: 'Erro',
                        detail: 'Erro ao cadastrar perfil!'+error.data.message
                    });
                })
        }else{
            perfilService.alterar(perfil)
               .then((response) => {
                    ...
                    toast.current?.show({
                        severity: 'error',
                        summary: 'Erro',
                        detail: 'Erro ao alterar perfil!'+error.data.message
                    });
                })
        }
    };
            ...
            perfilService.excluir(perfil.id)
              .then((response) => {
                    ...
                    toast.current?.show({
                        severity: 'error',
                        summary: 'Erro',
                        detail: 'Erro ao excluir perfil!'+error.data.message
                    });
                })
        }
    };
    ...
    const deleteSelectedPerfis = () => {
        Promise.all(
            selectedPerfis.map(async(_perfil) => {
                if(_perfil.id){
            ...
            toast.current?.show({
                severity: 'error',
                summary: 'Erro',
                detail: 'Erro ao excluir perfis!'+error.data.message,
                life: 3000
            });
        })
    };
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    >
                        ...
                        <Column field="descricao" header="Nome" sortable body={descricaoBodyTemplate} headerStyle={{ minWidth: '15rem' }}></Column>
                    ...
                    <Dialog visible={perfilDialog} style={{ width: '450px' }} header="Detalhes de perfil" modal className="p-fluid" footer={perfilDialogFooter} onHide={hideDialog}>
                        <div className="field">
                            ...
                            />
                            {submitted && !perfil.descricao && <small className="p-invalid">Nome é obrigatório.</small>}
                        </div>
                    </Dialog>
                    ...
                    <Dialog visible={deletePerfilDialog} style={{ width: '450px' }} header="Confirmar" modal footer={deletePerfilDialogFooter} onHide={hideDeletePerfilDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {perfil && (
                                <span>
                                    Realmente deseja excluir o perfil? <b>{perfil.descricao}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
                    ...
                    <Dialog visible={deletePerfisDialog} style={{ width: '450px' }} header="Confirmar" modal footer={deletePerfisDialogFooter} onHide={hideDeletePerfisDialog}>
                        <div className="flex align-items-center justify-content-center">
                            <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                            {perfil && <span>Realmente deseja excluir os perfis?</span>}
                        </div>
                    </Dialog>
                </div>
            </div>
