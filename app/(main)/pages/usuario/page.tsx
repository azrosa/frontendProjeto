            ...
            usuarioService.inserir(usuario)
                .then((response) => {
                    ...
                    toast.current?.show({
                        severity: 'error',
                        summary: 'Erro',
                        detail: 'Erro ao cadastrar usuário!'+error.data.message
                    });
                })
        }else{
    ...
    const deleteSelectedUsuarios = () => {
        Promise.all(
            ...
            toast.current?.show({
                severity: 'error',
                summary: 'Erro',
                detail: 'Erro ao excluir usuários!'+error.data.message,
                life: 3000
            });
        })
    };
                ...
                <div className="card">
                    ...
                    <Dialog visible={deleteUsuarioDialog} style={{ width: '450px' }} header="Confirmar" modal footer={deleteUsuarioDialogFooter} onHide={hideDeleteUsuarioDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {usuario && (
                                <span>
                                    Realmente deseja excluir o usuário? <b>{usuario.nome}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
                    ...
                    <Dialog visible={deleteUsuariosDialog} style={{ width: '450px' }} header="Confirmar" modal footer={deleteUsuariosDialogFooter} onHide={hideDeleteUsuariosDialog}>
                        <div className="flex align-items-center justify-content-center">
                            <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                            {usuario && <span>Realmente deseja excluir os usuários?</span>}
                        </div>
                    </Dialog>
                </div>
            </div>
